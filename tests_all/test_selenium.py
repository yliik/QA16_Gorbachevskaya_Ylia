...
def driver_chrome():
    ...
    try:
        cookie_overlay = driver.find_element(By.XPATH, '//*[@id="modal-cookie"]/div')
        if cookie_overlay:
...
def test_product_search(driver_chrome):
    ...
    if current_url == "https://www.21vek.by/tv/":
...
def test_adding_in_cart(driver_chrome):
    ...
    time.sleep(5)
    try:
        notification_element = driver_chrome.find_element(By.XPATH,
                                                          '//*[@id="header"]/div/div[3]/div/div[4]/a/span[1]/span')
        assert notification_element.is_displayed(), "Товар не был добавлен в корзину"
    except NoSuchElementException:
        pytest.fail("Товар не был добавлен в корзину")
def test_social_media_button(driver_chrome):
...
def test_login_password(driver_chrome):
    ...
    time.sleep(5)
    find_and_click_element(driver_chrome, user_button_xpath)
    element_after_login = driver_chrome.find_element(By.XPATH, '//*[@id="userToolsDropDown"]/div/span')
    assert element_after_login